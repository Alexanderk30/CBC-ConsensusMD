# ConsensusMD — Agent System Prompts (CORRECTED v2)

**What changed from v1.** Removed all references to "reasoning_trace" and "scratchpad." The original prompts implied a free-text CoT field that does not exist in the schemas. This version correctly treats the structured `reasoning` field (ProbabilisticReasoning / MechanisticReasoning / EliminativeReasoning sub-schemas) as the chain-of-thought surface. Each specialist thinks *in its assigned frame*, not in free prose.

**Why this is actually better than the scratchpad approach.** A scratchpad lets the model reason in prose and then fill structured fields as a post-hoc summary. That defeats the purpose of structured reasoning frames — the three specialists would end up thinking identically and only differing in how they formatted their summaries. With structured reasoning as the only thinking surface, each specialist is forced to reason *in its frame* from the start, which is what makes the three agents genuinely different in practice.

**Other design choices (unchanged from v1):**
- Clinical/professional consult-note register throughout
- Antagonist is adversarial but fair with explicit reward for declaring no-credible-challenge
- Every prompt has a "binding rules" section at the end for reinforcement
- Every prompt forbids mentioning the multi-agent architecture in the output
...
## Summary of resolved decisions from Claude Code's Step 1 planning review

These are the architectural commitments made after reviewing Claude Code's questions. They supersede any ambiguity in earlier docs.

| # | Decision | Rationale |
|---|---|---|
| Q1 | Structured `reasoning` field IS the CoT. No separate `reasoning_trace`. | Forces genuine frame-specific reasoning, not post-hoc formalization. |
| Q2 | Only `survival_count >= 2` terminates debate. `consensus_state` is informational/UI only. | Antagonist survival is the epistemic criterion; specialist agreement is the display. |
| Q3 | Leading diagnosis algorithm: majority → highest commitment among tied → alphabetical → highest commitment across all differentials → alphabetical (fallback for 3-way split). | Guarantees non-null leading diagnosis even on 3-way splits, without injecting specialist ordering bias. |
| Q4 | `supporting_evidence_summary` dedupe via normalization (lowercase, strip punctuation, collapse whitespace), not exact string match. | Catches near-duplicates cheaply; deterministic. |
| Q5 | `position_deltas` are round-over-round (N-1 vs N-2), not cumulative. Empty in Round 1. | Cumulative deltas would drown signal in noise. |
| Q6 | Ship NarrativeCriteriaCheck only for MVP. Defer Wells/HEART/McDonald library to Thursday. `criteria_check` remains optional on Diagnosis. | Evidence triad (supporting/refuting/alternative) does the load-bearing work; criteria library is strengthening, not foundation. |

Non-blocking confirmation: `check_termination()` runs after antagonist call, before next specialist round. Converged Round N antagonist correctly skips Round N specialist call.
